...
- **چهره‌ی متحرک بدون blendshape**: چون خروجی Tripo مورف صورت ندارد، viewer
  هنگام لود، ناحیه‌ی دهان و چشم‌ها را از روی رنگ تکسچر (مردمک آبی، داخل دهان
  قرمز) پیدا می‌کند و دو morph target می‌سازد: باز شدن فک (لولایی) و پلک زدن
- **حالت صحبت کامل**: لیپ‌سینک لب‌ها با فایل صوتی یا میکروفون + پلک زدن
  خودکار تصادفی + ژست‌های خودکار سر و دست هنگام حرف زدن
- چرخش/زوم دوربین

API چهره:

```js
rahino.face.mouth(0.5)            // باز کردن دستی دهان (۰ تا ۱)
rahino.face.blink()               // یک پلک
rahino.face.autoBlink(false)      // خاموش/روشن کردن پلک خودکار
rahino.face.autoTalkGestures(false) // خاموش/روشن کردن ژست‌های حین صحبت
```
